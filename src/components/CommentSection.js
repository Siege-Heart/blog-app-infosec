...
const API_COMMENT_URL = 'http://localhost:5001/api/comments'; // PORT 5000 → 5001
const API_FETCH_COMMENTS_URL = 'http://localhost:5001/api/comments'; // PORT 5000 → 5001
...
export default function CommentSection({ postId, currentUser }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!newComment.trim() || !postId || submitting || !currentUser) {
            return;
        }
        ...
        try {
            ...
            const response = await fetch(API_COMMENT_URL, {
                ...
                body: JSON.stringify({
                    ...
                    // Note: Backend gets author from req.user, so this might not be needed
                }),
            });
